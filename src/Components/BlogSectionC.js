...
function BlogSection() {
  ...
  return (
    <div className="md:mx-20 text-center md:mt-[40px] md:py-2 border-b mx-6 px-4 py-8">
      ...
      <div className="grid md:grid-flow-col grid-flow-row items-center justify-center gap-[25px]">
        {
          data && data.slice(0, 3).map((datum) => (
            <div key={datum.id} className="grid grid-flow-row font-inter h-[550px] w-[300px] md:h-[500px] md:w-[390px] gap-[15px] p-[25px] rounded-[10px]">
              <img
                src={`${datum.img}`}
                alt="Building Renovation"
                className="h-[200px] w-[250px] md:h-[280px] md:w-[340px] rounded-[10px]"
              />
              <p className="font-semibold text-[18px] md:text-[22px] h-[50px] md:h-[80px] w-[250px] md:w-full">{truncateText(datum.title, 8)}</p>
              <p className="text-[#000000B3] text-[15px] md:text-[17px] h-[110px] md:h-[150px] w-[250px] md:w-full">{truncateText(datum.content, 15)}</p>
      ...
      <a href="/blog">
        <button className="bg-[#FC9713] text-black px-6 py-2 rounded-md mt-10 cursor-pointer md:my-[50px] md:mt-[220px] md:h-[50px] md:w-[240px] md:font-bold md:text-[20px]">
          More Articles
        </button>
      </a>
    </div>
